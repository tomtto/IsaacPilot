...
isaacsim
```
Первый запуск дольше обычного — Isaac Sim компилирует шейдеры и разворачивает кэш экстеншенов. Дальше открывается основное окно (Viewport, Stage, Property и т.д.).
...
Если что-то не так — см. таблицу проблем в основном `README.md` (раздел про частые ошибки) и `TROUBLESHOOTING.md`.
