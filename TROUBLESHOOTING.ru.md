...
### 22. Увеличенный прим дрона взлетает штатно, а потом кувыркается — масштаб ломает передаваемую ориентацию
**Симптом:** прим дрона увеличен в сцене (`Scale 5` на корне), чтобы аппарат размером с ладонь было видно на видео. Взлёт проходит нормально («Высота 4.70 м»), но через пару секунд дрона сносит вбок и переворачивает. В полётном логе моторы борются друг с другом: `pwm=[1825, 1470, 1950, 1150]` — часть в верхнем упоре, часть у холостых.

**Что отмели первым делом:** масса и инерция целы (PhysX показывает ровно авторские 1.5 кг и Iris-инерцию — масштаб прима не трогает явные массовые свойства); плечи винтов в расчёте момента — константы, от геометрии сцены не зависят; коллайдер вырос, но на управление не влияет.

**Причина:** `read_state()` брал ориентацию из **мировой матрицы** через `ExtractRotationQuat()`. Эта функция корректна только для матрицы **без scale/shear**. С масштабом кватернион выходит непропорциональным настоящему повороту, и после нормализации это уже *другой* поворот:

| реальный наклон | что уходит в ArduPilot (Scale 5) |
|---|---|
| 2° | 5.0° |
| 10° | 24.6° |
| 30° | 66.3° |

В горизонте ошибка ровно нулевая — поэтому взлёт и выглядит идеально. Дальше: дрон качнулся на 2° → автопилот видит 5° → парирует вдвое сильнее нужного → реальный наклон растёт → передаваемый растёт быстрее → раскачка и переворот за секунды. Чем больше масштаб, тем хуже (при Scale 6 наклон 10° уезжает в 27°).

Тот же вызов есть второй раз — в физическом колбэке, и там хуже: полученная матрица крутит момент в мировые оси **и** переводит угловую скорость в связанные (гироскоп, который уходит в ArduPilot). То есть врали одновременно угол, момент и гироскоп.

**Решение:** снимать масштаб перед извлечением поворота, в обоих местах:
```python
quat = transform.RemoveScaleShear().ExtractRotationQuat()
```
Проверено численно: при `Scale 1` результат не меняется (регрессии нет), при `Scale 5/6` углы возвращаются точными. После фикса увеличенный в 5 раз дрон висит с креном 0.06° и остаточной скоростью 0.012 м/с.

**Что НЕ пострадало и почему:** вектор тяги (`xf.TransformDir(...).GetNormalized()` — нормируется, направление выживает), позиция (равномерный масштаб не искажает трансляцию), скорости (читаются из `RigidBodyAPI`, а не из матрицы).

**Правило:** любая матрица из `ComputeLocalToWorldTransform()` может нести масштаб прима. Достаёшь из неё **поворот** — сначала `RemoveScaleShear()`. Для направлений безопасно `TransformDir()` + `GetNormalized()`.
